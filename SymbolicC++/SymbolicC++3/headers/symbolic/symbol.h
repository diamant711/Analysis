...
void Symbol::print(ostream &o) const
{
 ...
 if(!parameters.empty())
 {
  ...
  for(list<Symbolic>::const_iterator i=++parameters.begin();
      i!=parameters.end();i++)
  {
   o << ((i==parameters.begin()) ? "":","); 
   i->print(o);
  }
  o << ((type() == typeid(Symbol)) ? "]":")");
 }
...
int Symbol::commute(const Symbolic &s) const
{
 ...
 // only Symbols should be non-commutative
 if(type() != typeid(Symbol)) return 1;
 
 if(s.type() == typeid(Symbol))
  return commutes || CastPtr<const Symbol>(s)->commutes;
...
// since the overide on simplification and other methods
// will be lost
